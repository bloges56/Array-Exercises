...
export const getStudentsByInstructorId = (instuctorId) => {
    return students.filter(student => {
        return instuctorId === student.instructorId
    })
}
...
export const getPolyglotStudents = languageCount => {
    return students.filter(student => {
        return languageCount <= student.languages.length
    })
}
...
export const getAvailableInstructors = () => {
    return instructors.filter(instructor => {
        const studentsOfInstructor = students.filter(student => {
            return student.instructorId === instructor.id
        })
        return studentsOfInstructor.length === 0
    })
}
...
export const getStudentsByLanguage = language => {
    return students.filter(student => {
        return student.languages.some(studentLanguage => {
            return studentLanguage === language
        })
    })
}
...
export const getStudentsByLanguages = languages => {
    return students.filter(student => {
        for(let i = 0; i < languages.length; i++) {
            if(!(student.languages.includes(languages[i]))){
                return false
            }
        }
        return true
